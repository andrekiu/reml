module Matrix = {
  ...
  let dot = ((d_a, a): t, (d_b, b): t, fn): t =>
    fst(d_a) != fst(d_b)
      ...
      : make(
          Array.init(fst(d_a), r =>
            Array.init(snd(d_a), c => fn(a[r][c], b[r][c mod snd(d_b)]))
          ),
        );
  ...
  let reduce = ((_, m): t, fn) => {
    Array.fold_left(
      (sum, row) => Array.fold_left((sum, e) => fn(sum, e), sum, row),
      0.0,
      m,
    );
  };
  ...
  let reduce_rows = (((dr, _), m): t): t => {
    (
      (dr, 1),
      Array.init(dr, ix =>
        [|Array.fold_left((sum, e) => sum +. e, 0.0, m[ix])|]
      ),
    );
  };
  ...
  let add_col = (v, ((r, c), raw): t) => {
    (
      (r, c + 1),
      Array.init(r, ir =>
        Array.init(c + 1, ic => ic == 0 ? v : raw[ir][ic - 1])
      ),
    );
  };
  let raw_max_idx = [%raw
