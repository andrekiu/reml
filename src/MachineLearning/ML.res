...
let norm = (x: Matrix.t, ~ix: array<int>=[], ()) => {
  let aggregate = ((low, high, sum), e) => (min(low, e), max(high, e), sum +. e)
  let reduce = (fn, e, m) => {
    let rec loop = (ix, sum) =>
      switch ix {
      | ix if ix == m => sum
  ...
  let normalize_dim = (col: int, ans) => {
    ...
    ans => {
      let (m, _) = Matrix.size(ans)
      iter(ix => Matrix.set(ix, col, (Matrix.get(ix, col, ans) -. mean) /. range, ans), m)
    }
  }

  let ans = Matrix.make(snd(x))
  let (_, c) = Matrix.size(ans)
  let massage = Array.map(row_ix => {
...
let normalize = (points: array<(float, float)>) => {
  let aggregate = ((low, high, sum), e) => (min(low, e), max(high, e), sum +. e)
  let (x, y) = points[0]
  let (tx, _) = Array.fold_left(
    ((sumx, sumy), (x, y)) => (aggregate(sumx, x), aggregate(sumy, y)),
    ((x, x, 0.), (y, y, 0.)),
    points,
  )
  let get_u = ((_, _, sum)) => sum /. float_of_int(Array.length(points))
  let get_d = ((l, h, _)) => h -. l
  let transform = (t, p) => (p -. get_u(t)) /. get_d(t)

  let normalized = Array.map(((x, y)) => (transform(tx, x), y), points)

  let x_m = Matrix.make(
    Array.init(Array.length(normalized), r => Array.init(2, c => c == 0 ? 1. : normalized[r]->fst)),
  )
...
module LinReg = {
  ...
  let cost = (theta, x_m, y) => {
    ...
    let (m, _) = Matrix.size(x_m)
    Matrix.reduce(residual, (sum, e) => e ** 2.0 /. (2.0 *. float_of_int(m)) +. sum)
  }
}
...
module LogReg = {
  ...
  let gradient_step = (alpha, theta, x_m, y) => {
    ...
    Matrix.res(theta, scale)->Matrix.res(
      _,
  ...
  let cost = (theta, x_m, y) => {
    ...
    let pos = Matrix.dot(y, Matrix.transform(pred, (_, _, e) => log(e +. eps)), (a, b) =>
      a *. b /. float_of_int(m)
    )
    let neg = Matrix.dot(y, Matrix.transform(pred, (_, _, e) => log(1. -. e +. eps)), (a, b) =>
