module Fit = {
  ...
  let near_boundary = (x1, theta) => {
    let residual =
      LinAlg.Matrix.get(0, 0, theta) +. LinAlg.Matrix.get(1, 0, theta) *. x1;
  ...
  let get_color = (n, y) => {
    ...
    let rec loop = ix =>
      ...
      | ix when ix == cols => (-1)
      | ix =>
        int_of_float(LinAlg.Matrix.get(n, ix, y)) == 1 ? ix : loop(ix + 1)
      };
    loop(0);
  };
      ...
      {(ctx, scale) => {
         ...
         let points =
           Array.init(rows, ix =>
             (
               (
                 LinAlg.Matrix.get(ix, 1, x_m),
                 LinAlg.Matrix.get(ix, 2, x_m),
               ),
               get_color(ix, y),
             )
           );
         ...
         Array.iter(
           p =>
             Stylus.draw_point(ctx, t(fst(p)), ~color=pallete[snd(p)], ()),
           points,
         );
         ...
         forn(
           ...
           ix => {
             let line =
               List.map(
                 e =>
                   (e, near_boundary(e, LinAlg.Matrix.get_col(ix, theta)))
                   ->t,
                 [low, high],
               );
...
module Train = {
  let getRandomTheta = y => {
    ...
    LinAlg.Matrix.make(
      Array.init(3, _ => Array.init(pred, _ => Random.float(400.))),
    );
  };
    ...
    React.useEffect1(
      () => {
        Js.Global.setTimeout(
          () => {
            let next = ML.LogReg.gradient_step(20., theta, x_m, y);
            setTheta(_ => next);
            ();
          },
          16,
...
let make = () => {
  ...
  let y =
    LinAlg.Matrix.make(
      Array.init(len, row =>
        Array.init(clusters, col => snd(points[row]) - col == 0 ? 1.0 : 0.0)
      ),
    );
  <Train x_m y />;
};