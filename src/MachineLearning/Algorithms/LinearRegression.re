module HypothesisFit = {
  ...
  let make = (~x_m, ~y, ~theta) => {
    ...
    let points =
      React.useMemo(() => {
        Array.init(Matrix.size(x_m)->fst, ix =>
          (Matrix.get(ix, 1, x_m), Matrix.get(ix, 0, y))
        )
      });
    let dims = (400, 400);
    <Canvas dims>
      {(ctx, scale) => {
         let h = x =>
           Matrix.get(0, 0, theta) +. x *. Matrix.get(1, 0, theta);
         let (_, (low, high)) = scale(points, dims);
         let get_bounds = x => (x, h(x));
         let extremes = [get_bounds(low), get_bounds(high)];
         let re =
           scale(Array.append(Array.of_list(extremes), points), dims);
         let t = fst(re);
...
module TimeTravel = {
  ...
  let make = (~hist_theta, ~onSelect) => {
    ...
    <div>
      <div> {React.string({j|Converged in $iters steps.|j})} </div>
      <div
           ...
           List.mapi(
             (ix, (t, cost)) =>
               <span
                 ...
                 {React.string(
                    Js.Float.toFixedWithPrecision(cost, ~digits=5),
                  )}
               </span>,
             hist_theta,
           )
...
module FitPoly = {
  let is_stable = (hist): bool =>
    ...
    || (
      switch (hist) {
      | [a, b, ..._] => abs_float(snd(a) -. snd(b)) < 0.00001
      | _ => false
      }
    );
  ...
  let get_initial_state = (x_m, y): state => {
    let theta =
      LinAlg.Matrix.vector([|Random.float(400.), Random.float(400.)|]);
    {
      hist_theta: [(theta, ML.LinReg.cost(theta, x_m, y))],
      selection: None,
      alpha: 2.,
    };
  };
              ...
              () =>
                setState(s => {
                  ...
                  let next = ML.LinReg.gradient_step(s.alpha, prev, x_m, y);
                  {
                    ...s,
                    hist_theta: [
                      (next, ML.LinReg.cost(next, x_m, y)),
                      ...s.hist_theta,
                    ],
                  };
                }),
      ...
      <div
        ...
        <span>
          ...
          {let theta = get_theta(state);
           let (m0, m1) = (
             LinAlg.Matrix.get(0, 0, theta),
             LinAlg.Matrix.get(1, 0, theta),
           );
           let sign = m1 < 0.0 ? "-" : "+";
           let str_m0 = Js.Float.toFixedWithPrecision(m0, ~digits=2);
           let abs_m1 =
             abs_float(m1)->Js.Float.toFixedWithPrecision(_, ~digits=2);
           React.string({j|$str_m0 $sign x * $abs_m1|j})}
        </span>
        <span>
        ...
        <TimeTravel
          ...
          onSelect={selection =>
            setState(s => {...s, selection: Some(selection)})
          }
        />
        <CostCovergence
          hist_theta={
            ...
            | Some((ix, _)) =>
              Belt.List.take(
                List.rev(state.hist_theta),
                List.length(state.hist_theta) - ix,
              )
              ->Belt.Option.getExn
              ->List.rev
            }
